-- =============================================================================
-- Able Pro IAM - Production Final Schema
-- MySQL 8.0+ | Schema: smart_real_time_analyser
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- Drop order (respect FK dependencies)
-- -----------------------------------------------------------------------------
...
-- is_active: master switch | mfa_enabled/mfa_secret: TOTP
-- -----------------------------------------------------------------------------
CREATE TABLE users (
...
-- -----------------------------------------------------------------------------
-- otp_codes (forgot-password flow, 5-min expiry)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- mfa_recovery_codes (10 hashed backup codes per user)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- refresh_tokens (linked to sessions)
-- -----------------------------------------------------------------------------
...
INSERT INTO permissions (id, slug, description, created_at, updated_at) VALUES
  (UNHEX(REPLACE(UUID(), '-', '')), 'user:read', 'View users', NOW(6), NOW(6)),
  (UNHEX(REPLACE(UUID(), '-', '')), 'user:write', 'Create and edit users', NOW(6), NOW(6)),
  (UNHEX(REPLACE(UUID(), '-', '')), 'user:create', 'Create users', NOW(6), NOW(6)),
  (UNHEX(REPLACE(UUID(), '-', '')), 'billing:view', 'View billing', NOW(6), NOW(6)),
  (UNHEX(REPLACE(UUID(), '-', '')), 'audit:view', 'View audit logs', NOW(6), NOW(6));
...
INSERT INTO roles (id, name, description, created_at, updated_at) VALUES
  (@role_admin_id, 'Admin', 'Administrator with full access', NOW(6), NOW(6)),
  (@role_user_id, 'User', 'Standard user', NOW(6), NOW(6));

-- Role-Permission mappings
-- Admin: user:read, user:write, user:create, billing:view, audit:view
INSERT INTO role_permissions (role_id, permission_id, created_at)
SELECT @role_admin_id, id, NOW(6) FROM permissions WHERE slug IN ('user:read', 'user:write', 'user:create', 'billing:view', 'audit:view');

-- User: user:read only
INSERT INTO role_permissions (role_id, permission_id, created_at)
SELECT @role_user_id, id, NOW(6) FROM permissions WHERE slug = 'user:read';

-- =============================================================================
-- End of PRODUCTION_FINAL_SCHEMA.sql
-- =============================================================================
